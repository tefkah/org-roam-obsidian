import { one } from './one'
import { J, MdastContent, Node, OrgastParent, Handle } from './types'

/**
 * Convert all nodes in tree using j
 * @param j orgast constructor function
 * @param parent
 * @returns
 */
export function all(j: J, parent: Node): Array<MdastContent> {
  // @ts-expect-error Assume `parent` is a parent.
  const nodes: Array<Node> = parent.children || []
  const values: Array<MdastContent> = []
  let index = -1
