...
where $C$ is the path traversed by the particle through some parameter space, $\lambda^i$ the parameter being varied and $\mathcal{A}_i$ the \_\_ for a specific quantum state $\ket{n(R)}$, defined as
